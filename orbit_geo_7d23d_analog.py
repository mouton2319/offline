...
def split_stems(
    ...
    split_mode: str,
    seed: int,
    ts_unix_map: Optional[Dict[str, float]] = None,
    ...
    if split_mode == "hash":
        ...
        if len(va) == 0 and len(tr) > 1:
            va = [tr.pop(-1)]
        return tr, va
    if ts_unix_map is not None:
        ss = sorted(stems, key=lambda s: (float(ts_unix_map.get(s, 0.0)), s))
    else:
        ss = sorted(stems)
...
def cyclic_day_distance(d1: np.ndarray | float, d2: np.ndarray | float, period: float = 366.0) -> np.ndarray:
    a = np.asarray(d1, dtype=np.float64)
    b = np.asarray(d2, dtype=np.float64)
    dd = np.abs(a - b)
    return np.minimum(dd, float(period) - dd)


def ts_to_year_doy(ts: dt.datetime) -> Tuple[int, float]:
    y = int(ts.year)
    day0 = int(ts.timetuple().tm_yday)
    frac = (float(ts.hour) * 3600.0 + float(ts.minute) * 60.0 + float(ts.second)) / 86400.0
    return y, float(day0) + frac


def normalize_optional_positive(value: Optional[float]) -> Optional[float]:
    if value is None:
        return None
    v = float(value)
...
def select_knn_indices_and_weights(
    d_row: np.ndarray,
    ord_idx_row: np.ndarray,
    k: int,
    tau: float,
    cand_ts_unix: Optional[np.ndarray],
    cand_year: Optional[np.ndarray],
    cand_doy: Optional[np.ndarray],
    query_ts_unix: Optional[float],
    query_year: Optional[int],
    query_doy: Optional[float],
    year_half_life: Optional[float],
    doy_sigma: Optional[float],
    causal: bool,
    pool_mult: int,
) -> Tuple[np.ndarray, np.ndarray]:
    d_row = np.asarray(d_row, dtype=np.float64).reshape(-1)
    ord_idx = np.asarray(ord_idx_row, dtype=np.int64).reshape(-1)
    kk = max(1, int(k))
    pm = max(1, int(pool_mult))

    cand = ord_idx.copy()
    if causal and cand_ts_unix is not None and query_ts_unix is not None:
        ts_arr = np.asarray(cand_ts_unix, dtype=np.float64).reshape(-1)
        m = ts_arr[cand] <= float(query_ts_unix)
        cand = cand[m]
    if len(cand) == 0:
        cand = ord_idx.copy()
    pool_n = min(len(cand), max(kk, kk * pm))
    pool = cand[:pool_n]
    if len(pool) == 0:
        pool = ord_idx[: max(1, kk)]

    dd = d_row[pool]
    tt = max(1e-6, float(tau))
    logw = -dd / tt

    yhl = normalize_optional_positive(year_half_life)
    if yhl is not None and cand_year is not None and query_year is not None:
        yy = np.asarray(cand_year, dtype=np.float64).reshape(-1)[pool]
        lag = np.maximum(0.0, float(query_year) - yy)
        logw = logw - (lag / float(yhl))

    dsg = normalize_optional_positive(doy_sigma)
    if dsg is not None and cand_doy is not None and query_doy is not None:
        ddoy = cyclic_day_distance(float(query_doy), np.asarray(cand_doy, dtype=np.float64).reshape(-1)[pool], period=366.0)
        logw = logw - 0.5 * ((ddoy / float(dsg)) ** 2)

    top = np.argsort(logw)[::-1][:kk]
    idx = pool[top]
    lw = logw[top]
    lw = lw - float(np.max(lw))
    w = np.exp(lw)
    sw = float(np.sum(w))
    if sw <= 1e-12 or not np.all(np.isfinite(w)):
        w = np.ones((len(idx),), dtype=np.float64) / max(1, len(idx))
    else:
        w = w / sw
...
def build_train_samples(
    ...
    for rec in records:
        ...
        y0, doy0 = ts_to_year_doy(rec.ts)
        ts_unix = float(rec.ts.timestamp())
...
def train_cmd(args: argparse.Namespace) -> int:
    ...
    stem_ts_map = {s.stem: float(s.ts_unix) for s in samples}
    tr_stems, va_stems = split_stems(
        stems=stems,
        val_split=float(args.val_split),
        split_mode=str(args.split_mode),
        seed=int(args.seed),
        ts_unix_map=stem_ts_map,
    )
    tr_set = set(tr_stems)
    va_set = set(va_stems)
    ...
    year_half_life_grid = parse_float_csv(args.year_half_life_grid)
    if len(year_half_life_grid) == 0:
        year_half_life_grid = [0.0]
    doy_sigma_grid = parse_float_csv(args.doy_sigma_grid)
    if len(doy_sigma_grid) == 0:
        doy_sigma_grid = [0.0]
    pool_mult = max(1, int(args.neighbor_pool_mult))
    causal_train = bool(args.causal_train)
    ...
    tr_ts = np.asarray([s.ts_unix for s in tr], dtype=np.float64)
    tr_year = np.asarray([s.year for s in tr], dtype=np.float64)
    tr_doy = np.asarray([s.doy for s in tr], dtype=np.float64)
    va_ts = np.asarray([s.ts_unix for s in va], dtype=np.float64)
    va_year = np.asarray([s.year for s in va], dtype=np.int64)
    va_doy = np.asarray([s.doy for s in va], dtype=np.float64)

    for k in k_grid:
        kk = max(1, min(int(k), Xtr_z.shape[0]))
        for tau in tau_grid:
            for yhl in year_half_life_grid:
                yhl_eff = normalize_optional_positive(float(yhl))
                for dsg in doy_sigma_grid:
                    dsg_eff = normalize_optional_positive(float(dsg))
                    pred_coef_az = np.zeros((len(va), Ctr_az.shape[1]), dtype=np.float64)
                    pred_coef_el = np.zeros((len(va), Ctr_el.shape[1]), dtype=np.float64)
                    for i in range(len(va)):
                        idx_i, w_i = select_knn_indices_and_weights(
                            d_row=D[i],
                            ord_idx_row=ord_idx[i],
                            k=kk,
                            tau=float(tau),
                            cand_ts_unix=tr_ts,
                            cand_year=tr_year,
                            cand_doy=tr_doy,
                            query_ts_unix=float(va_ts[i]),
                            query_year=int(va_year[i]),
                            query_doy=float(va_doy[i]),
                            year_half_life=yhl_eff,
                            doy_sigma=dsg_eff,
                            causal=causal_train,
                            pool_mult=pool_mult,
                        )
                        pred_coef_az[i] = np.sum(Ctr_az[idx_i] * w_i.reshape(-1, 1), axis=0)
                        pred_coef_el[i] = np.sum(Ctr_el[idx_i] * w_i.reshape(-1, 1), axis=0)
                    ...
                    obj = 0.5 * (c_az + c_el)
                    yhl_log = "none" if yhl_eff is None else f"{float(yhl_eff):g}"
                    dsg_log = "none" if dsg_eff is None else f"{float(dsg_eff):g}"
                    log(
                        f"[SEARCH] k={kk} tau={float(tau):g} yhl={yhl_log} doy_sigma={dsg_log} | "
                    ...
                    if obj < best_obj:
                        ...
                        best = {
                            "k": int(kk),
                            "tau": float(tau),
                            "year_half_life": (None if yhl_eff is None else float(yhl_eff)),
                            "doy_sigma": (None if dsg_eff is None else float(dsg_eff)),
                            "alpha_az": float(a_az),
                            "alpha_el": float(a_el),
    ...
    meta = {
        ...
        "selected_year_half_life": best.get("year_half_life"),
        "selected_doy_sigma": best.get("doy_sigma"),
        "causal_train": bool(causal_train),
        "neighbor_pool_mult": int(pool_mult),
    ...
    np.savez_compressed(
        ...
        sample_ts_unix=np.asarray([s.ts_unix for s in samples], dtype=np.float64),
        sample_year=np.asarray([s.year for s in samples], dtype=np.int16),
        sample_doy=np.asarray([s.doy for s in samples], dtype=np.float32),
        sample_truth_key=np.asarray([s.truth_key for s in samples]),
    )
    summary = {
        "selected_k": int(best["k"]),
        "selected_tau": float(best["tau"]),
...
def predict_cmd(args: argparse.Namespace) -> int:
    ...
    if "sample_ts_unix" in arrays and len(arrays["sample_ts_unix"]) == len(Xbank_z):
        sample_ts_unix = arrays["sample_ts_unix"].astype(np.float64)
    else:
        ts_fallback: List[float] = []
        for s in stem_list:
            ss = s.split("::", 1)[-1]
            tt = parse_stem_datetime(ss)
            ts_fallback.append(float(tt.timestamp()) if tt is not None else 0.0)
        sample_ts_unix = np.asarray(ts_fallback, dtype=np.float64)
    if "sample_year" in arrays and len(arrays["sample_year"]) == len(Xbank_z):
        sample_year = arrays["sample_year"].astype(np.float64)
    else:
        sample_year = np.asarray(
            [float(dt.datetime.fromtimestamp(float(t)).year) if np.isfinite(t) and t > 0 else 0.0 for t in sample_ts_unix],
            dtype=np.float64,
        )
    if "sample_doy" in arrays and len(arrays["sample_doy"]) == len(Xbank_z):
        sample_doy = arrays["sample_doy"].astype(np.float64)
    else:
        doy_fallback: List[float] = []
        for t in sample_ts_unix:
            if np.isfinite(t) and t > 0:
                tt = dt.datetime.fromtimestamp(float(t))
                _, dy = ts_to_year_doy(tt)
                doy_fallback.append(float(dy))
            else:
                doy_fallback.append(1.0)
        sample_doy = np.asarray(doy_fallback, dtype=np.float64)

    k = int(args.k if args.k is not None else meta["selected_k"])
    tau = float(args.tau if args.tau is not None else meta["selected_tau"])
    year_half_life = normalize_optional_positive(
        args.year_half_life if args.year_half_life is not None else meta.get("selected_year_half_life")
    )
    doy_sigma = normalize_optional_positive(
        args.doy_sigma if args.doy_sigma is not None else meta.get("selected_doy_sigma")
    )
    causal_predict = bool(args.causal_predict)
    neighbor_pool_mult = int(
        max(
            1,
            args.neighbor_pool_mult if args.neighbor_pool_mult is not None else int(meta.get("neighbor_pool_mult", 8)),
        )
    )
    ...
    q_ts = parse_stem_datetime(tle_file.stem)
    if q_ts is None:
        q_ts = dt.datetime.now()
    q_year, q_doy = ts_to_year_doy(q_ts)
    q_ts_unix = float(q_ts.timestamp())
    ...
    idx, w = select_knn_indices_and_weights(
        d_row=d,
        ord_idx_row=ord_idx,
        k=kk,
        tau=float(tau),
        cand_ts_unix=sample_ts_unix,
        cand_year=sample_year,
        cand_doy=sample_doy,
        query_ts_unix=float(q_ts_unix),
        query_year=int(q_year),
        query_doy=float(q_doy),
        year_half_life=year_half_life,
        doy_sigma=doy_sigma,
        causal=causal_predict,
        pool_mult=neighbor_pool_mult,
    )
    tt = max(1e-6, float(tau))
        ...
        if use_rows >= obs_min_valid and len(pre_unix) >= use_rows:
            ...
            try:
                ...
                if obs_valid_rows >= obs_min_valid:
                    cand = ord_idx
                    if causal_predict:
                        m_c = sample_ts_unix[cand] <= float(q_ts_unix)
                        cand = cand[m_c]
                    if len(cand) == 0:
                        cand = ord_idx
                    pool = cand[: min(len(cand), int(obs_pool_size))]
                    ...
                    if np.any(np.isfinite(e)):
                        ...
                        if year_half_life is not None:
                            lag = np.maximum(0.0, float(q_year) - sample_year[pool])
                            logw = logw - lag / float(year_half_life)
                        if doy_sigma is not None:
                            ddoy = cyclic_day_distance(float(q_doy), sample_doy[pool], period=366.0)
                            logw = logw - 0.5 * ((ddoy / float(doy_sigma)) ** 2)
    ...
    pred_df["year_half_life"] = (np.nan if year_half_life is None else float(year_half_life))
    pred_df["doy_sigma"] = (np.nan if doy_sigma is None else float(doy_sigma))
    pred_df["causal_predict"] = bool(causal_predict)
    pred_df["neighbor_pool_mult"] = int(neighbor_pool_mult)
    ...
    log(
        ...
        f"yhl={'none' if year_half_life is None else f'{year_half_life:g}'} "
        f"doy_sigma={'none' if doy_sigma is None else f'{doy_sigma:g}'} "
        f"causal={causal_predict} "
...
def make_parser() -> argparse.ArgumentParser:
    ...
    t.add_argument("--year-half-life-grid", type=str, default="0,1,2,4,8",
                   help="Year recency half-life grid. <=0 disables recency weighting.")
    t.add_argument("--doy-sigma-grid", type=str, default="0,15,30,60,120",
                   help="Day-of-year Gaussian sigma grid (days). <=0 disables seasonal weighting.")
    t.add_argument("--causal-train", action=argparse.BooleanOptionalAction, default=True,
                   help="During validation search, only allow analogs not newer than query timestamp.")
    t.add_argument("--neighbor-pool-mult", type=int, default=8,
                   help="Candidate pool size = k * neighbor-pool-mult for reweighting.")
    ...
    pr.add_argument("--year-half-life", type=float, default=None,
                    help="Year recency half-life. <=0 disables recency weighting.")
    pr.add_argument("--doy-sigma", type=float, default=None,
                    help="Day-of-year Gaussian sigma (days). <=0 disables seasonal weighting.")
    pr.add_argument("--causal-predict", action=argparse.BooleanOptionalAction, default=True,
                    help="Only allow analogs not newer than query TLE timestamp.")
    pr.add_argument("--neighbor-pool-mult", type=int, default=None,
                    help="Candidate pool size = k * neighbor-pool-mult for reweighting.")
